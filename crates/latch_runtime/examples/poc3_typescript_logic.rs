...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for tick in 0..3 {
        ...
        let (total_entities, modified_positions, modified_velocities) =
            runtime.context.with(|ctx| {
                ...
                bounce_fn.call::<_, ()>((pos_array, vel_array.clone(), total_entities, bounds))?;
    ...
    for &arch_id in position_archs {
        ...
        if let (Some(positions), Some(velocities)) = (
            ...
            for i in 0..positions.len().min(5) {
                println!(
                    "   Entity {}: pos=({}, {}), vel=({}, {})",
                    entity_count, positions[i].x, positions[i].y, velocities[i].x, velocities[i].y
                );
                entity_count += 1;
            }
        }
