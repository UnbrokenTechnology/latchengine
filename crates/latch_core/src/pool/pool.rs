...
pub enum PoolError {
    ...
    RangeCrossesPage { start: usize, end: usize, rows_per_page: usize },
...
impl<T> PagedPool<T> {
    ...
    fn localize_range(&self, range: Range<usize>) -> Result<(usize, Range<usize>), PoolError> {
        if range.start > range.end {
            return Err(PoolError::RangeOutOfBounds { start: range.start, end: range.end, len: self.len_total() });
        }
        ...
        if p0 != p1 {
            return Err(PoolError::RangeCrossesPage { start: range.start, end: range.end, rows_per_page: self.rows_per_page });
        }
    ...
    fn ensure_page_with_space(&mut self) -> usize {
        if let Some((idx, _)) = self.pages.iter().enumerate().rev().find(|(_, p)| !p.is_full()) {
            idx
        } else {
    ...
    pub fn alloc_one(&mut self) -> usize {
        ...
        let local = self.pages[pid].alloc_one().expect("page should have capacity");
    ...
    pub fn alloc_bulk(&mut self, mut count: usize) -> Vec<Range<usize>> {
        ...
        while count > 0 {
            ...
            let local = page.alloc_bulk(take).expect("allocation within page must succeed");
    ...
    pub fn slice_tile(&self, range: Range<usize>) -> Result<&[T], PoolError> {
        ...
        let page_ref = self.pages.get(page).ok_or(PoolError::PageMissing { page })?;
        page_ref.slice(local)
    }

    pub fn slice_tile_mut(&mut self, range: Range<usize>) -> Result<&mut [T], PoolError> {
        let (page, local) = self.localize_range(range)?;
        let page_ref = self.pages.get_mut(page).ok_or(PoolError::PageMissing { page })?;
    ...
    pub fn free_one_swap_remove(
        ...
        let page = self.pages.get_mut(page_idx).ok_or(PoolError::PageMissing { page: page_idx })?;
    ...
    pub fn free_bulk_swap_remove(
        ...
        while cursor < gidxs.len() {
            ...
            let locals: Vec<usize> = gidxs[start..cursor].iter().map(|&g| self.local_of(g)).collect();
            let page = self.pages.get_mut(page_idx).ok_or(PoolError::PageMissing { page: page_idx })?;
